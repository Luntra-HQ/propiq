 ...
 *
 * FIX: Using <span tabIndex={0}> instead of <button> to avoid Radix event loop
 * Credit: Grok AI analysis - button + preventDefault causes infinite re-renders
 */
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <span
          tabIndex={0}
          className={`inline-flex items-center justify-center w-4 h-4 cursor-help ${className}`}
        >
          <HelpCircle className="w-4 h-4 text-gray-400 hover:text-violet-400 transition-colors" />
        </span>
      </TooltipTrigger>
      <TooltipContent
        side="right"
